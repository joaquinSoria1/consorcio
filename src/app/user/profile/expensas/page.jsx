...
export default function Dashboard() {
    ...
    const [expensasData, setExpensasData] = useState(null);
    const imagenPerfil = userData?.imagenPerfil?.data;
    const expensas = session?.user?.expensas;
    ...
    useEffect(() => {
        if (session && session.user && session.user.inquilinos && session.user.inquilinos.length > 0) {
            ...
            axios.get('/api/expensa')
                .then(response => {
                    const filteredExpensas = response.data.filter(expensa => expensas.includes(expensa._id));
                    setExpensasData(filteredExpensas);
                })
                .catch(error => {
    ...
    return (
        <div className="grid h-screen w-full box-content">
            ...
            <main className="px-10">
                <Inicio userData={userData} expensasData={expensasData}></Inicio>
            </main>
        </div>
    )
}
